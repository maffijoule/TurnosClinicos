...
// Reusa SH, LW, EJ_COLORS, DIAS, DIAS_SHORT, slot2str, tieneColacion, horasTrabajadas de solver.js
  ...
  DIAS.forEach((dia, di) => {
    tms.forEach((tm, tmIdx) => {
      ...
      block.innerHTML = `
        ...
        ${tieneColacion(d.dur) ? `<div class="img-tb-col" data-tm="${tm.nombre}" data-dia="${dia}"
...
function imgTotalHorasSem() {
  ...
  return imgGetTMs().reduce((s, tm) =>
    s + Object.values(sem[tm.nombre] || {}).filter(d => d.activo)
      .reduce((t, d) => t + horasTrabajadas(d.dur), 0), 0);
}
  ...
  panel.innerHTML = `<div style="font-size:10px;font-weight:700;color:var(--muted);
    ...
    imgGetTMs().map(tm => {
      const h   = Object.values(sem[tm.nombre] || {}).filter(d => d.activo)
                    .reduce((s, d) => s + horasTrabajadas(d.dur), 0);
    ...
    imgGetTMs().forEach(tm => {
      DIAS.forEach(dia => {
        ...
        rows.push([
          ...
          horasTrabajadas(d.dur),
          tieneColacion(d.dur) ? '30 min' : '—',
        ]);
      });
    });
