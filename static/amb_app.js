// ── CEM Capacity Planner — amb_app.js ────────────────────────────────────────
// Módulo Ambulatorio: estado, carga de Excel, cálculo de demanda, UI
// ─────────────────────────────────────────────────────────────────────────────
...
const S = {
  ...
  modoEspera: false, resultado: null, demandaAvg: null,
};
...
const C = {
  mes: null, anio: null,
  puestos_max: 4, turno_min: 4, turno_max: 9, horas_min_ejec: 20,
  apertura_min: 2, cierre_min: 2, apertura_min_sat: 2, cierre_min_sat: 2,
  hora_apertura: 7, hora_cierre: 21, hora_apertura_sat: 9, hora_cierre_sat: 14,
};
...
function toggleModoEspera() {
  ...
  if (S.modoEspera) {
    tog.style.background = 'var(--accent)'; dot.style.left = '18px';
    std.style.opacity = '0.35'; std.style.pointerEvents = 'none';
    esp.style.display = 'block';
  } else {
    tog.style.background = 'var(--border2)'; dot.style.left = '2px';
    std.style.opacity = '1'; std.style.pointerEvents = 'auto';
...
function parseDemanda(raw) {
  ...
  const outputDows = DOW_ORDER.filter(d =>
    d !== 'domingo' && horas.some(h => acc[h][d] && acc[h][d].some(v => v > 0))
  );
  S.raw = acc; S.horas = horas; S.dows = outputDows;
}
...
function calcular() {
  ...
  S.demandaAvg = demandaAvg; S.resultado = resultado;
  renderResultado();
...
function heatClass(v, max) {
  ...
  const r = v / max;
  if (r < 0.15) return 'h1'; if (r < 0.30) return 'h2';
  if (r < 0.50) return 'h3'; if (r < 0.70) return 'h4';
  return 'h5';
}
...
function renderResultado() {
  ...
  const kpiHTML = `<div class="kpi-row">
    <div class="kpi-card"><div class="kpi-label">Demanda total</div>
      <div class="kpi-val">${totalDem.toLocaleString('es-CL')}</div>
      <div class="kpi-sub">personas promedio / semana</div></div>
    <div class="kpi-card blue"><div class="kpi-label">Demanda máxima</div>
      <div class="kpi-val">${globalMax.toLocaleString('es-CL', {minimumFractionDigits:1,maximumFractionDigits:1})}</div>
      <div class="kpi-sub">ejecutivos simultáneos pico</div></div>
    <div class="kpi-card green"><div class="kpi-label">Días de semana</div>
      <div class="kpi-val">${dows.length}</div><div class="kpi-sub">lunes a viernes</div></div>
    <div class="kpi-card amber"><div class="kpi-label">Franjas horarias</div>
      <div class="kpi-val">${horas.length}</div><div class="kpi-sub">con demanda registrada</div></div>
  </div>`;
  ...
  const dowHTML = `<div class="table-card">
    ...
    <div class="dow-grid">
      ${dows.map((d, i) => {
        const s = dowStats[i]; const bar = (s.max / globalMax * 100).toFixed(1);
  ...
  const capRes = Math.min(C.puestos_max, E.ejecutivos.length || C.puestos_max);
  const trows = horas.map(h => {
    const cells = dows.map(d => {
      const v = resultado[h][d]; const excede = v > capRes;
      return `<td class="${excede ? '' : heatClass(v, globalMax)}"
        style="${excede ? 'color:#c0392b;font-weight:700;background:#fff0f0' : ''}">
        ${excede ? `${fmt(v)} ⚠` : fmt(v)}</td>`;
    }).join('');
    const rowMax = Math.max(...dows.map(d => resultado[h][d])); const rowExcede = rowMax > capRes;
    return `<tr><td class="td-hora">${h}</td>${cells}
      <td class="td-max" style="${rowExcede ? 'color:#c0392b;font-weight:700' : ''}">${fmt(rowMax)}${rowExcede ? ' ⚠' : ''}</td></tr>`;
  }).join('');

  const hayExceso = horas.some(h => dows.some(d => resultado[h][d] > capRes));
  const warningBanner = hayExceso ? `<div style="background:#fff3cd;border:1px solid #ffc107;border-radius:var(--radius);
    ...
    <div><strong>Demanda supera la capacidad máxima real</strong><br>
    <span style="color:var(--muted)">Celdas en rojo exceden min(puestos, ejecutivos) = ${capRes}. El solver usará ese valor como techo.</span></div>
  </div>` : '';

  const tableHTML = `${warningBanner}<div class="table-card">
    <div class="table-card-header"><div>
      <div class="table-card-title">Personal Requerido por Hora</div>
      <div class="table-card-sub">Ejecutivos simultáneos necesarios · ⚠ = supera min(puestos, ejecutivos) = ${capRes}</div>
    </div>
    ...
    <div class="table-scroll"><table>
      <thead><tr><th class="th-hora">Hora</th>${thDays}<th class="th-max">Demanda máxima</th></tr></thead>
      <tbody>${trows}</tbody>
    </table></div></div>`;
...
function exportarExcel() {
  ...
  const r1 = horas.map(h => { const vals = dows.map(d => parseFloat(resultado[h][d].toFixed(4))); return [h, ...vals, parseFloat(Math.max(...vals).toFixed(4))]; });
  ...
  const ws3 = XLSX.utils.aoa_to_sheet(S.modoEspera ? [
    ['Parámetro','Valor'],['Modo','Tiempo de espera'],
    ['Tiempo de examen (min)',params.tiempo_examen],['Espera máxima (min)',params.espera_max],
    [],['Fórmula','TMs = (D_avg × Esp_max / 60) / (⌊Esp_max / T_exam⌋ + 1)'],
  ] : [
    ['Parámetro','Valor'],['Modo','Estándar'],
    ['Tiempo de atención (min)',params.tiempo],['Productividad',params.prod],
...
function renderEjecutivosContent() {
  ...
  if (!E.ejecutivos.length) {
    content.innerHTML = `<div class="empty"><div class="empty-icon">👥</div>
      ...
      <div class="empty-sub">Carga el Excel de ejecutivos o descarga el template para comenzar.</div></div>`;
    return;
  }
  ...
  const kpiHTML = `<div class="kpi-row">
    <div class="kpi-card"><div class="kpi-label">Ejecutivos</div>
      <div class="kpi-val">${E.ejecutivos.length}</div>
      <div class="kpi-sub">${nFull} full · ${nPart} part-time</div></div>
    <div class="kpi-card blue"><div class="kpi-label">Horas totales / semana</div>
      <div class="kpi-val">${totalHoras.toLocaleString('es-CL')}</div>
      <div class="kpi-sub">horas semanales suma</div></div>
    <div class="kpi-card green"><div class="kpi-label">Período</div>
      <div class="kpi-val" style="font-size:18px">${C.mes ? MESES[C.mes] : '—'}</div>
      <div class="kpi-sub">${C.anio || '—'}</div></div>
    <div class="kpi-card"><div class="kpi-label">Puestos físicos</div>
      <div class="kpi-val">${C.puestos_max}</div><div class="kpi-sub">máx simultáneos</div></div>
  </div>`;
  const rows = E.ejecutivos.map(ej => {
    ...
    return `<tr>
      ...
      <td><div style="font-family:var(--mono);font-size:12px;font-weight:600">${ej.horas_semana}h/sem</div>
        <div class="horas-bar-wrap"><div class="horas-bar ${barClass}" style="width:${pct}%"></div></div></td>
    </tr>`;
  }).join('');
  ...
  content.innerHTML = kpiHTML + `<div class="table-card">
    <div class="table-card-header"><div>
...
function renderConfigContent() {
  ...
  if (!C.mes || !C.anio) {
    content.innerHTML = `<div class="empty"><div class="empty-icon">⚙️</div>
      ...
      <div class="empty-sub">Ingresa el mes y año en el panel izquierdo para ver el calendario del período.</div></div>`;
    return;
  }
  ...
  let startDow = cur.getDay() === 0 ? 6 : cur.getDay() - 1;
  for (let i = 0; i < startDow; i++) semana.push(null);
  for (let d = 1; d <= nDays; d++) { semana.push(d); if (semana.length===7){semanas.push(semana);semana=[];} }
  if (semana.length) semanas.push(semana);
  const calHTML = `<div class="table-card">
    <div class="table-card-header"><div class="table-card-title">Calendario ${MESES[C.mes]} ${C.anio}</div></div>
    <div style="padding:16px;overflow-x:auto">
      <table style="border-collapse:collapse;width:100%;font-size:12px;text-align:center">
        <thead><tr>${['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'].map(d => `<th style="padding:6px 10px;color:var(--muted);font-size:11px;font-weight:700;letter-spacing:.5px">${d}</th>`).join('')}</tr></thead>
...
function descargarTemplate() {
  ...
  const wsDem = XLSX.utils.aoa_to_sheet([
    ['*** HOJA DEMANDA ***','','',''],['Esta hoja la genera tu sistema de datos.','','',''],
    ['Estructura requerida:','','',''],['Fila 1:','Días →','Número del día del mes (1, 2, 3...)',''],
    ['Fila 2:','Horas →','Nombre del día de semana (lunes, martes...)',''],
    ['Fila 3+:','Hora del día','Demanda en esa franja horaria',''],['','','',''],
    ['Ejemplo:','','',''],['Días',1,2,3],['Horas','lunes','martes','miércoles'],
    [8,12,19,17],[9,30,27,23],[10,34,33,34],
  ]);
  wsDem['!cols'] = [{wch:14},{wch:14},{wch:36},{wch:14}];
  XLSX.utils.book_append_sheet(wb, wsDem, 'Demanda');
  const wsEj = XLSX.utils.aoa_to_sheet([
    ['Nombre','Horas_semana','Tipo'],['Ejecutivo 1',45,'full'],['Ejecutivo 2',45,'full'],
    ['Ejecutivo 3',45,'full'],['Ejecutivo 4',45,'full'],['Ejecutivo 5',25,'part'],['Ejecutivo 6',25,'part'],
  ]);
  wsEj['!cols'] = [{wch:22},{wch:14},{wch:10}];
  XLSX.utils.book_append_sheet(wb, wsEj, 'Ejecutivos');
  const wsCfg = XLSX.utils.aoa_to_sheet([
    ['Parámetro','Valor','Descripción'],['Mes',6,'Número de mes (1-12)'],['Año',2025,''],
    ...
    ['Ejecutivos_apertura_min',2,'Mínimo al abrir'],['Ejecutivos_cierre_min',2,'Mínimo al cerrar'],
...
async function ejecutarSolver() {
  ...
  btn.disabled = true;
  try {
    const ping = await fetch('/amb/ping', { signal: AbortSignal.timeout(8000) });
    if (!ping.ok) throw new Error('Servidor no responde');
  } catch (e) {
    status.innerHTML = `❌ Solver no disponible.<br>
      <span style="color:var(--text2)">Ejecuta <b>python app.py</b> primero.</span>`;
  ...
  try {
    const t0 = Date.now();
    const res = await fetch('/amb/solve', {
    ...
    if (data.status === 'infeasible') {
      ...
      renderInfeasible(data); btn.disabled = false; return;
    }
    S.solverResult = data;
...
function buildSolverPayload() {
  ...
  for (const dow of S.dows) {
    ...
    for (const hora of S.horas) {
      ...
      if (val > 0) demanda[dow][hora] = Math.min(val, C.puestos_max, E.ejecutivos.length);
    }
  }
  return {
    ejecutivos: E.ejecutivos, demanda,
    configuracion: {
      puestos_fisicos_max: C.puestos_max, turno_min_horas: C.turno_min, turno_max_horas: C.turno_max,
      apertura_min: C.apertura_min, cierre_min: C.cierre_min,
      apertura_min_sat: C.apertura_min_sat, cierre_min_sat: C.cierre_min_sat,
      hora_apertura: C.hora_apertura, hora_cierre: C.hora_cierre,
      hora_apertura_sat: C.hora_apertura_sat, hora_cierre_sat: C.hora_cierre_sat,
...
function renderInfeasible(data) {
  ...
  const causas = (data.causas_posibles || []).map(c =>
    `<div style="padding:10px 14px;border-left:3px solid var(--danger,#c0392b);
      background:#fff5f5;border-radius:0 6px 6px 0;font-size:12px;line-height:1.6;margin-bottom:8px">${c}</div>`
  ).join('');
  ...
  content.innerHTML = `
    ...
    <div class="table-card">
      <div class="table-card-header"><div class="table-card-title">💡 Contexto actual</div></div>
